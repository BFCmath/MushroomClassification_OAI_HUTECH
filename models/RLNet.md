...
[MK BLOCK 1]
    ...
    Layer 3 - Final Aggregation:
                    ...
                    MaxPool (2x2, stride=2)
                        (16x16x24)
    ↓
[MK BLOCK 2] - Same structure but with (24→24) input channels
    Multi-path feature extraction → Feature combinations → Aggregation → MaxPool
    (8x8x24)
    ↓
[MK BLOCK 3] - Same structure but with (24→24) input channels
    Multi-path feature extraction → Feature combinations → Aggregation → MaxPool
    (4x4x24)
    ↓
[FINAL PROCESSING]
...
Each Multi-Kernel (MK) Block follows this structure:
